...
console.log(limit, 'limit');
    ...
    const getFeatures = async () => {
        try {
            ...
            const response = await apiCall({
                method: "GET",
                endpoint: `${API_ENDPOINTS.Features}?${queryParams}`,
            });
        ...
        try {
            await apiCall({
                ...
                endpoint: selectedFeature
                    ? `${API_ENDPOINTS.Features}/${selectedFeature._id}`
    ...
    const handleDelete = async (featureId: string) => {
        ...
        await apiCall({
            method: "DELETE",
            endpoint: `${API_ENDPOINTS.Features}/${featureId}`,
        });
                                ...
                                <tr key={features._id}>
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap">
                                        <div className="flex items-center">
                                            {getStatusIcon(features.status)}
                                            <span className={`ml-2 px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${getStatusClass(features.status)}`}>
                                                {features.status.charAt(0).toUpperCase() + features.status.slice(1)}
                                            </span>
                                        </div>
                                    </td>
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        <div className="flex space-x-3 gap-5">
                                            ...
                                            <button
                                                onClick={() => openDeleteModal(features)}
